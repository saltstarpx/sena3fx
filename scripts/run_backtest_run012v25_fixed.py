...
from strategies.yagami_mtf_v25 import generate_signals
...
class FixedBacktestEngine(BacktestEngine):
    ...
    def run(self, data, signal_func, sl_series, m1_upper, m1_lower, freq='1min', name='Strategy', **kwargs):
        self.trades = []
        ...
        for i in range(1, len(data)):
            ...
            for trade in self.trades:
                if trade['status'] == 'OPEN':
                    ...
                    # 利益確定判定 (BB 2.5シグマ)
                    if trade['direction'] == 'LONG' and curr_price >= m1_upper.iloc[i]:
                        should_exit = True
                    elif trade['direction'] == 'SHORT' and curr_price <= m1_lower.iloc[i]:
                        should_exit = True
...
def main():
    ...
    print("シグナル生成中...")
    signal_series, sl_series, m1_upper, m1_lower, signals_detail = generate_signals(df, df_15m, df_4h, spread=0.01)
    ...
    results = engine.run(df, signal_func, sl_series, m1_upper, m1_lower, name="Yagami_v25_Fixed")
    
    print("\n" + "=" * 80)
    print("【RUN-012v25-Fixed】ストレス・テスト結果")
    print("=" * 80)
    ...
    if results['total_trades'] > 0:
        ...
        output_csv = os.path.join(BASE_DIR, 'results', 'run012v25_trades.csv')
