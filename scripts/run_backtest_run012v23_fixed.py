    ...
    def run(self, data, signal_func, sl_series, freq='1min', name='Strategy', **kwargs):
        ...
        for i in range(1, len(data)):
            ...
            if sig != 0:
                ...
                # スプレッドを考慮したエントリー価格
                if direction == 'LONG':
                    entry_price += self.slippage_pips / 100 # 1.0pips = 0.01
                else:
...
def main():
    ...
    results = engine.run(df, signal_func, sl_series, name="Yagami_v23_Fixed")
    
    print("\n" + "=" * 80)
    print("【RUN-012v23-Fixed】ストレス・テスト結果")
    print("=" * 80)
    ...
    if results['total_trades'] > 0:
        ...
        output_csv = os.path.join(BASE_DIR, 'results', 'run012v23_trades.csv')
