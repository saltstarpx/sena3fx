...
from strategies.yagami_mtf_v21 import generate_signals, calculate_bb
...
class FixedBacktestEngine(BacktestEngine):
    ...
    def run(self, data, signal_func, sl_series, freq='1min', name='Strategy', **kwargs):
        ...
        bb_upper, bb_lower = calculate_bb(data, period=20, std_dev=2)
            ...
            for trade in self.trades:
                if trade['status'] == 'OPEN':
                    ...
                    if trade['direction'] == 'LONG':
                        # 利益確定: BBアッパー + 0.02 にタッチ
                        if curr_price >= bb_upper.iloc[i] + 0.02:
                            should_exit = True
                        # 損切り: SLにタッチ
                        elif curr_price <= trade['sl']:
                            should_exit = True
                    else:
                        # 利益確定: BBロワー - 0.02 にタッチ
                        if curr_price <= bb_lower.iloc[i] - 0.02:
                            should_exit = True
                        # 損切り: SLにタッチ
                        elif curr_price >= trade['sl']:
                            should_exit = True
                    
                    # 48時間経過しても決済されない場合は強制決済
...
def main():
    ...
    print("シグナル生成中...")
    signal_series, sl_series, signals_detail = generate_signals(df, df_15m, df_4h, spread=0.01)
    ...
    results = engine.run(df, signal_func, sl_series, name="Yagami_v21_Fixed")
    
    print("\n" + "=" * 80)
    print("【RUN-012v21-Fixed】ストレス・テスト結果")
    print("=" * 80)
