...
from yagami_mtf_v62 import generate_signals, calculate_atr # v62のgenerate_signalsをインポート
...
def run_backtest(symbol, start_date, end_date, spread_pips):
    ...
    stop_loss = 0
    risk_amount = 0
    original_risk_amount = 0 # 半利確前のリスク額
    ...
    for i in range(len(data_1m)):
        ...
        if current_position != 0:
            ...
            if not half_position_closed:
                if current_position == 1 and current_price >= half_tp:
                    pnl_pips = (half_tp - entry_price) * 100 # 1pips = 0.01 (USDJPY)
                    trades.append({
                        ...
                        'ATR_at_Entry': atr_at_entry_series.loc[entry_time]
                    })
                    # 残りのポジションのSLを建値に移動
                    stop_loss = entry_price
                    half_position_closed = True
                    # print(f"[{current_time}] LONG HALF CLOSED. PnL: {pnl_pips:.2f} pips. SL moved to entry price: {stop_loss:.5f}")
                elif current_position == -1 and current_price <= half_tp:
                    ...
                    stop_loss = entry_price
                    half_position_closed = True
                    # print(f"[{current_time}] SHORT HALF CLOSED. PnL: {pnl_pips:.2f} pips. SL moved to entry price: {stop_loss:.5f}")
            ...
            if (current_position == 1 and current_bar['low'] <= stop_loss) or \
               (current_position == -1 and current_bar['high'] >= stop_loss):
                ...
                # print(f"[{current_time}] {'LONG' if current_position == 1 else 'SHORT'} SL HIT. PnL: {pnl_pips:.2f} pips. Price: {current_price:.5f}")
                current_position = 0
                entry_price = 0
                entry_time = None
                take_profit = 0
            ...
            if (current_position == 1 and current_bar['high'] >= take_profit) or \
               (current_position == -1 and current_bar['low'] <= take_profit):
                ...
                # print(f"[{current_time}] {'LONG' if current_position == 1 else 'SHORT'} TP HIT. PnL: {pnl_pips:.2f} pips. Price: {current_price:.5f}")
                current_position = 0
                entry_price = 0
                entry_time = None
                take_profit = 0
        ...
        if current_position == 0:
            ...
            if signal != 0:
                ...
                # print(f"[{current_time}] {'LONG' if current_position == 1 else 'SHORT'} Entry. Price: {entry_price:.5f}, TP: {take_profit:.5f}, SL: {stop_loss:.5f}, Risk: {risk_amount:.5f}")
...
if __name__ == "__main__":
    ...
    # バックテスト実行
    trades_v62 = run_backtest(SYMBOL, START_DATE, END_DATE, SPREAD_PIPS)

    # 結果の保存
    output_dir = "/home/ubuntu/sena3fx/results"
    os.makedirs(output_dir, exist_ok=True)
    trades_v62.to_csv(os.path.join(output_dir, "trades_v62.csv"), index=False)

    print(f"Backtest completed. Results saved to {os.path.join(output_dir, 'trades_v62.csv')}")

    # 統計情報の表示
    if not trades_v62.empty:
        total_pnl = trades_v62['PnL_pips'].sum()
        num_trades = len(trades_v62)
        winning_trades = trades_v62[trades_v62['PnL_pips'] > 0]
        losing_trades = trades_v62[trades_v62['PnL_pips'] < 0]
        ...
        if avg_loss != 0:
            ...
            print("Kelly Criterion: Cannot calculate (Average Loss is zero)")
