...
from yagami_mtf_v63 import generate_signals, calculate_atr
...
if __name__ == "__main__":
    ...
    trades_v63 = run_backtest(SYMBOL, START_DATE, END_DATE, SPREAD_PIPS)

    output_dir = "/home/ubuntu/sena3fx/results"
    os.makedirs(output_dir, exist_ok=True)
    trades_v63.to_csv(os.path.join(output_dir, "trades_v63.csv"), index=False)

    print(f"Backtest completed. Results saved to {os.path.join(output_dir, 'trades_v63.csv')}")

    if not trades_v63.empty:
        total_pnl = trades_v63['PnL_pips'].sum()
        num_trades = len(trades_v63[~trades_v63['Position'].str.contains('HALF')]) # 決済単位ではなくエントリー単位でカウント
        winning_trades = trades_v63[trades_v63['PnL_pips'] > 0]
        losing_trades = trades_v63[trades_v63['PnL_pips'] < 0]
