...
from strategies.yagami_mtf_v23 import generate_signals

class FixedBacktestEngine(BacktestEngine):
    def __init__(self, init_cash=1000000, slippage_pips=0.2, **kwargs):
        super().__init__(init_cash=init_cash, slippage_pips=slippage_pips, **kwargs)
        self.init_cash = init_cash
        self.slippage_pips = slippage_pips

    def run(self, data, signal_func, sl_series, freq='1min', name='Strategy', **kwargs):
        self.trades = []
        ...
        for i in range(1, len(data)):
            ...
            if sig != 0:
                ...
                # スプレッドを考慮したエントリー価格 (0.2pips = 0.002)
                if direction == 'LONG':
                    entry_price += self.slippage_pips / 100
                else:
                    entry_price -= self.slippage_pips / 100
                
                sl_price = sl_series.loc[data.index[i]]
...
def main():
    ...
    print("シグナル生成中...")
    signal_series, sl_series, signals_detail = generate_signals(df, df_15m, df_4h, spread=0.002)
    print(f"生成されたシグナル: {len(signals_detail)} 件")
    
    engine = FixedBacktestEngine(init_cash=1000000, slippage_pips=0.2)
    ...
    results = engine.run(df, signal_func, sl_series, name="Yagami_v23_LowSpread")
    
    print("\n" + "=" * 80)
    print("【RUN-012v23-LowSpread】ストレス・テスト結果")
    print("=" * 80)
    ...
    if results['total_trades'] > 0:
        ...
        output_csv = os.path.join(BASE_DIR, 'results', 'run012v23_low_spread_trades.csv')
