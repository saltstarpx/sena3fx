    ...
    def run(self, data, signal_func, freq='1min', name='Strategy', **kwargs):
        ...
        for i in range(1, len(data)):
            ...
            if sig != 0:
                ...
                # エントリー価格にスプレッド分を加味
                # USDJPYの場合、1.0 pips = 0.01
                entry_price = data.iloc[i]['close']
                if direction == 'LONG':
                    entry_price += 0.01 # 1.0 pips
                else:
                    entry_price -= 0.01 # 1.0 pips
            ...
            # 決済ロジック（15分後に決済して十分な値幅が出ているか確認）
            for trade in self.trades:
                if trade['status'] == 'OPEN':
                    # エントリーから15分経過したか確認
                    time_diff = (data.index[i] - trade['entry_time']).total_seconds() / 60
                    if time_diff >= 15:
                        exit_price = data.iloc[i]['close']
                        if trade['direction'] == 'LONG':
                            pnl = (exit_price - trade['entry_price']) * 10000 # 1万通貨
                        else:
...
def main():
    ...
    print("シグナル生成中...")
    signal_series, _ = generate_signals(df, df_15m, df_4h, spread=0.01)
    ...
    results = engine.run(df, signal_func, name="Yagami_v6_Fixed")
    
    print("\n" + "=" * 80)
    print("【RUN-012v6-Fixed】ストレス・テスト結果")
    print("=" * 80)
