...
from strategies.yagami_mtf_v19 import generate_signals, calculate_bb
                ...
                if trade['status'] == 'OPEN':
                    ...
                    if trade['direction'] == 'LONG':
                        ...
                        if curr_price >= bb_upper.iloc[i]:
                            should_exit = True
                        # 下位バンドにタッチしたら損切り
                        elif curr_price <= bb_lower.iloc[i]:
                            should_exit = True
                    else:
                        # 下位バンドにタッチしたら利益確定
                        if curr_price <= bb_lower.iloc[i]:
                            should_exit = True
                        # 上位バンドにタッチしたら損切り
                        elif curr_price >= bb_upper.iloc[i]:
                            should_exit = True
...
def main():
    ...
    results = engine.run(df, signal_func, name="Yagami_v19_Fixed")
    
    print("\n" + "=" * 80)
    print("【RUN-012v19-Fixed】ストレス・テスト結果")
    print("=" * 80)
