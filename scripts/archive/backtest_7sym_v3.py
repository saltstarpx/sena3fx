...
backtest_7sym_v3.py
===================
7銘柄 3ヶ月バックテスト【yagami_position8_risk10_v3】
銘柄: USDJPY, AUDUSD, EURJPY, EURGBP, US30, SPX500, XAUUSD
期間: 2025-03-03 〜 2025-06-02
初期資金: 100万円

v3変更点（v2からの差分）:
  ① 含み益ポジションは枠カウント外（v2と同じ）
  ② 強制カット発動閾値を 0R（含み損転換瞬間）→ -0.5R（リスク額の50%超過）に緩和
  ③ 全カット後4時間クールタイム（v2と同じ）
"""
...
def simulate_single(symbol, signals, data_1m, init_cash, base_risk):
    ...
    for sig in signals:
        ...
        # risk_jpy: SL到達時の損失額（強制カット閾値計算用）
        risk_jpy = arm.calc_pnl_jpy(direction, ep, sl, lot, USDJPY_RATE, ep)
        risk_jpy = abs(risk_jpy)  # 正の値に変換
        trades.append({
            ...
            "risk_jpy": risk_jpy,  # SL到達時の損失額（強制カット閾値用）
...
def apply_position_filter_v2(all_trades_df, init_cash, base_risk):
    ...
    for current_ts in all_times_set:
        ...
        # 3. 損失転換チェック → 追加ポジション全カット（v3: -0.5R閘値）
        to_cut = pm.check_and_cut(now=current_ts, cut_threshold_r=-0.5)
        if to_cut:
        ...
        for idx, row in entry_candidates.iterrows():
            ...
            # risk_jpy: simulate_singleで計算したSL到達時の損失額を使用
            risk_jpy_val = float(row.get("risk_jpy", 0.0))
            pm.open_position(sym, eff_risk, entry_price=row["ep"],
                             entry_time=current_ts, is_extra=is_extra,
                             risk_jpy=risk_jpy_val)
    ...
    print(f"  強制カット発生回数: {cut_count}回（閾値: -0.5R）")
    return pd.DataFrame(accepted_trades), pd.Series(equity_curve)


# ── メイン ────────────────────────────────────────────────
print("=" * 70)
print(f"7銘柄 3ヶ月バックテスト【yagami_position8_risk10_v3】")
...
print("\n" + "=" * 70)
print(f"【v3 結果サマリー（{START} 〜 {END}）】")
print("=" * 70)
print(f"  採用トレード数:        {n_total}件（TP:{n_tp} SL:{n_sl} BE:{n_be}）")
print(f"  うち追加エントリー:    {n_extra}件（強制カット閾値: -0.5R）")
...
fig.suptitle(f"7銘柄バックテスト【v3: yagami_position8_risk10_v3】\n{START}〜{END}  初期資金: {INIT_CASH:,}円", fontsize=13, fontweight="bold")
...
plt.tight_layout()
out_path = os.path.join(OUT_DIR, "7sym_v3_result.png")
plt.savefig(out_path, dpi=150, bbox_inches="tight"); plt.close()
print(f"\nチャート保存: {out_path}")

accepted_df.to_csv(os.path.join(OUT_DIR, "7sym_v3_trades.csv"), index=False)
np.save(os.path.join(OUT_DIR, "7sym_v3_equity.npy"), eq_arr)
print("完了。")
