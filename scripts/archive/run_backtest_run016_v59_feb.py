...
def run_backtest(csv_path, spread_pips=1.0):
    ...
    signal_series, tp_series, sl_series, entry_time_series = generate_signals(df, data_15m, data_4h, spread)
    ...
    pd.DataFrame(trades).to_csv(f"/home/ubuntu/sena3fx/results/run016_v59_{os.path.basename(csv_path)}_trades.csv", index=False)
