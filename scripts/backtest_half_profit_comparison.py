"""
backtest_half_profit_comparison.py
====================================
半利確あり vs なしの比較バックテスト

【目的】
  本番（Cloud Run）が半利確なしで動いているため、
  半利確ON/OFFの影響を定量的に比較する。

【対象銘柄】XAUUSD / GBPUSD / EURUSD / AUDUSD
【ロジック】Logic-A（Goldロジック）統一
【IS/OOS】
...
SYMBOLS = ["XAUUSD", "GBPUSD", "EURUSD", "AUDUSD"]
...
def load_all(sym):
    """data/ の小文字ファイル（全期間連結）を使用"""
    sym_lower = sym.lower()
    def _f(tf):
        # まず全期間ファイルを探す
        p = os.path.join(DATA_DIR, f"{sym_lower}_{tf}.csv")
        if os.path.exists(p):
            first_line = open(p).readline()
            if "version" not in first_line:  # LFSポインタでないことを確認
                return load_csv(p)
        # ohlc/大文字も試す
        p2 = os.path.join(DATA_DIR, "ohlc", f"{sym}_{tf}.csv")
        if os.path.exists(p2):
            first_line = open(p2).readline()
...
# ── シグナル生成（Logic-A: Goldロジック） ──────────────────────────
def generate_signals(d1m_oos, d15m_oos, d4h_full, spread, atr_1m_d, m1c):
...
# ── EXIT: 半利確あり ──────────────────────────────────────────────
def _find_exit_with_half(highs, lows, ep, sl, tp, risk, direction):
...
# ── EXIT: 半利確なし（本番と同じ） ─────────────────────────────────
def _find_exit_no_half(highs, lows, ep, sl, tp, risk, direction):
    lim = min(len(highs), MAX_LOOKAHEAD)
    for i in range(lim):
        h = highs[i]; lo = lows[i]
        if direction == 1:
...
# ── シミュレーション ──────────────────────────────────────────────
def simulate(signals, d1m, sym, use_half_profit=True):
    ...
    exit_fn = _find_exit_with_half if use_half_profit else _find_exit_no_half
    ...
    for sig in signals:
        ...
        ei, xp, result, half_done = exit_fn(m1h[sp:], m1l[sp:],
                                             sig["ep"], sig["sl"], sig["tp"],
                                             sig["risk"], sig["dir"])
        if result is None: continue

        half_pnl = 0.0
        if half_done and use_half_profit:
...
# ── メイン ───────────────────────────────────────────────────────
def run_period(d1m_full, d15m_full, d4h_full, sym, start, end, use_half_profit):
    ...
    sigs  = generate_signals(d1m, d15m, d4h_full, spread, atr_1m, m1c)
    trades, final_eq, mdd, _ = simulate(sigs, d1m, sym, use_half_profit=use_half_profit)
...
def main():
    print("\n" + "="*80)
    print("  半利確あり vs なし 比較バックテスト（Goldロジック統一）")
    ...
    for sym in SYMBOLS:
        ...
        for half_label, use_half in [("半利確あり", True), ("半利確なし", False)]:
            oos_st = run_period(d1m_full, d15m_full, d4h_full, sym,
                                OOS_START, OOS_END, use_half)
            is_st  = run_period(d1m_full, d15m_full, d4h_full, sym,
                                IS_START, IS_END, use_half)

            results.append({
                "sym": sym, "half": use_half, "label": half_label,
                "oos": oos_st, "is": is_st
            })

    # ── 結果表示 ──────────────────────────────────────────────────
    print("\n" + "="*80)
    print("  ■ OOS結果比較（半利確あり vs なし）")
    print(f"  {'銘柄':8} {'モード':10} | {'n':>5} {'WR':>7} {'PF':>7} {'MDD':>7} {'損益':>12}")
    print("-"*80)

    for sym in SYMBOLS:
        sym_res = [r for r in results if r["sym"] == sym]
        if not sym_res: continue
        for r in sym_res:
            st = r["oos"]
            if not st:
                print(f"  {sym:8} {r['label']:10} | {'N/A':>5}")
                continue
            pf_s = f"{st['pf']:.2f}" if st['pf'] < 99 else "∞"
            print(f"  {sym:8} {r['label']:10} | {st['n']:>5} {st['wr']*100:>6.1f}% "
                  f"{pf_s:>7} {st['mdd']:>6.1f}% {st['total']:>+12,.0f}")
        print()

    # ── 差分サマリー ──────────────────────────────────────────────
    print("="*80)
    print("  ■ 半利確の影響サマリー（OOS）")
    print(f"  {'銘柄':8} | {'PF(あり)':>9} {'PF(なし)':>9} {'差':>7} | {'WR(あり)':>9} {'WR(なし)':>9}")
    print("-"*80)

    for sym in SYMBOLS:
        sym_res = [r for r in results if r["sym"] == sym]
        if len(sym_res) < 2: continue
        r_on  = next((r for r in sym_res if r["half"]), None)
        r_off = next((r for r in sym_res if not r["half"]), None)
        if not r_on or not r_off or not r_on["oos"] or not r_off["oos"]: continue

        pf_on  = r_on["oos"]["pf"]
        pf_off = r_off["oos"]["pf"]
        wr_on  = r_on["oos"]["wr"] * 100
        wr_off = r_off["oos"]["wr"] * 100
        diff   = pf_on - pf_off

        print(f"  {sym:8} | {pf_on:>9.2f} {pf_off:>9.2f} {diff:>+7.2f} | "
              f"{wr_on:>8.1f}% {wr_off:>8.1f}%")

    print("\n  ※ 本番（Cloud Run）は現在「半利確なし」で稼働中")
    print("  ※ 差が大きい場合は半利確ロジックの本番実装を推奨")
    print("="*80)
