...
from strategies.yagami_mtf_v49 import generate_signals
...
class OptimizedBacktestEngine:
    ...
    def run(self, data, signals, name='Strategy'):
        self.trades = []
        self.cash = self.init_cash
        
        print(f"Starting Optimized Backtest: {name} ({len(data)} bars)")
        
        # シグナルを時刻で辞書化して高速化
        signal_dict = {}
        for sig in signals:
            if sig['time'] not in signal_dict:
                signal_dict[sig['time']] = []
            signal_dict[sig['time']].append(sig)
        ...
        for i in range(1, len(data)):
            ...
            # 新規エントリー
            if curr_time in signal_dict:
                for sig in signal_dict[curr_time]:
...
def main():
    ...
    print("シグナル生成中...")
    signals = generate_signals(df, df_15m, df_4h, spread=0.01)
    print(f"生成されたシグナル: {len(signals)} 件")
    
    engine = OptimizedBacktestEngine(init_cash=1000000, slippage_pips=1.0)
    
    results = engine.run(df, signals, name="Yagami_v49_Optimized")
    
    print("\n" + "=" * 80)
    print("【RUN-012v49-Optimized】ストレス・テスト結果")
    print("=" * 80)
    ...
    if results['total_trades'] > 0:
        ...
        output_csv = os.path.join(BASE_DIR, 'results', 'run012v49_trades.csv')
