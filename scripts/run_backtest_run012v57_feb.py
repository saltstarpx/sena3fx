...
def run_backtest(csv_path, spread=0.01):
    ...
    for signal in signals_list:
        ...
        for t, bar in post_entry_data.iterrows():
            if direction == 'LONG':
                if bar['low'] <= sl:
                    pnl = (sl - entry_price) * 100 - (spread * 100)
                    exit_time = t
                    exit_price = sl
                    break
                elif bar['high'] >= tp:
    ...
    pd.DataFrame(trades).to_csv("/home/ubuntu/sena3fx/results/run012v57_feb_trades.csv", index=False)

if __name__ == "__main__":
    run_backtest(
        "/home/ubuntu/sena3fx/data/ohlc/USDJPY_1m_2026_Feb.csv",
        spread=0.01
    )
