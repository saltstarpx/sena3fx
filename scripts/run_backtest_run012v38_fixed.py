...
from strategies.yagami_mtf_v38 import generate_signals
...
class FixedBacktestEngine(BacktestEngine):
    ...
    def run(self, data, signal_func, tp_series, sl_series, freq='1min', name='Strategy', **kwargs):
        self.trades = []
        ...
        for i in range(1, len(data)):
            ...
            if sig != 0:
                ...
                tp_price = tp_series.loc[data.index[i]]
...
def main():
    ...
    results = engine.run(df, signal_func, tp_series, sl_series, name="Yagami_v38_Fixed")
    
    print("\n" + "=" * 80)
    print("【RUN-012v38-Fixed】ストレス・テスト結果")
    print("=" * 80)
    ...
    if results['total_trades'] > 0:
        ...
        output_csv = os.path.join(BASE_DIR, 'results', 'run012v38_trades.csv')
