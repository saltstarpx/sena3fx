...
def analyze_volatility(csv_path, label):
    ...
    print(f"--- {label} ---")
    print(f"Average 15m ATR: {atr:.4f} (approx {atr*100:.1f} pips)")
    print(f"Max 15m Range: {ranges.max(axis=1).max():.4f}")
    print(f"Spread 1.0pips impact: {0.01 / atr * 100:.1f}% of avg 15m range")
