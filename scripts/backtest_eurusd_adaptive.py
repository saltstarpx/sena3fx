"""
backtest_eurusd_adaptive.py
============================
EURUSD 1銘柄集中バックテスト（アダプティブリスク 2〜3%）
FX銘柄 Goldロジック順位 第3位（OOS PF=1.73）の検証

【設定】
  - 銘柄: EURUSD のみ
  - スプレッド: 0.0pips（OANDAプロコース生スプレッド）
...
SPREAD_PIPS          = 0.0
COMMISSION_USD_PER_LOT = 5.0

def calc_commission_jpy(lot):
...
def simulate(signals, d15m, risk_pct):
    ...
    for sig in signals:
        ...
        equity -= commission; total_commission += commission
        ...
        equity += pnl
        trades.append({"result": result, "pnl": half_pnl+pnl, "commission": commission})
...
def simulate_adaptive(signals, d15m):
    ...
    for sig in signals:
        ...
        trades.append({"result": result, "pnl": total_pnl, "commission": commission, "risk_used": rm.risk_pct})
        peak = max(peak, equity); mdd = max(mdd, (peak-equity)/peak*100)
    return trades, equity, mdd, total_commission
...
def calc_stats(trades):
    ...
    result = {"n": len(df), "wr": len(wins)/len(df),
...
def run_period(d15m_full, d4h_full, start, end, risk_pct, adaptive=False):
    ...
    if d15m is None or len(d15m) == 0: return {}, 0.0
    ...
    if st:
        st["mdd"] = mdd; st["final_eq"] = final_eq
        st["commission"] = total_comm; st["multiplier"] = final_eq / INIT_CASH
...
def monthly_pnl_adaptive(d15m_full, d4h_full):
    ...
    current_risk = RISK_INIT
    rm = RiskManager(SYM, risk_pct=current_risk)
    m15t = d15m_oos.index; m15h = d15m_oos["high"].values; m15l = d15m_oos["low"].values
    equity = INIT_CASH; results = {}
    for sig in sigs:
        rm.risk_pct = current_risk
        lot = rm.calc_lot(equity, sig["risk"], sig["ep"], usdjpy_rate=USDJPY_RATE)
        if lot <= 0: continue
...
def main():
    print("\n" + "="*80)
    print("  EURUSD 集中戦略バックテスト（アダプティブリスク 2〜3%, 0.5%刻み）")
    print(f"  FX Goldロジック 第3位（OOS PF=1.73）")
    ...
    print(sep)
    row("トレード数",    lambda s: f"{s['n']}",                        lambda s: f"{s['n']}")
    row("勝率",         lambda s: f"{s['wr']*100:.1f}%",              lambda s: f"{s['wr']*100:.1f}%")
    row("PF",           lambda s: f"{s['pf']:.2f}" if s['pf']<99 else "∞", lambda s: f"{s['pf']:.2f}" if s['pf']<99 else "∞")
    row("MDD",          lambda s: f"{s['mdd']:.1f}%",                 lambda s: f"{s['mdd']:.1f}%")
    row("資産倍率",      lambda s: f"{s['multiplier']:.2f}x",          lambda s: f"{s['multiplier']:.2f}x")
    row("最終資産(万円)", lambda s: f"{s['final_eq']/10000:.0f}万",     lambda s: f"{s['final_eq']/10000:.0f}万")
    row("手数料合計",    lambda s: f"{s['commission']/10000:.0f}万",    lambda s: f"{s['commission']/10000:.0f}万")
    ...
    if oos_ada and oos_ada.get("avg_risk"):
        ...
        if oos_ada.get("risk_dist"):
            ...
            for rv, cnt in sorted(oos_ada["risk_dist"].items()):
                print(f"    {rv*100:.1f}%: {cnt:>4}件  {'█'*min(int(cnt/10)+1,30)}")

    print("\n" + "-"*80)
    print("  ■ 過学習チェック（OOS PF / IS PF）")
    for label, is_st, oos_st in [("固定2%", is_2, oos_2), ("固定3%", is_3, oos_3), ("アダプティブ", is_ada, oos_ada)]:
        if is_st and oos_st and is_st.get("pf") and oos_st.get("pf"):
            r = oos_st["pf"] / is_st["pf"]
            print(f"    {label:12}: OOS/IS={r:.2f}  {'✅ 過学習なし' if r >= 0.7 else '⚠️ 過学習疑い'}")
    ...
    for ym in sorted(monthly.keys()):
        data = monthly[ym]; total = sum(data["pnls"]); cnt = len(data["pnls"])
        avgr = sum(data["risks"]) / cnt if cnt else 0
        sign = "+" if total >= 0 else ""; flag = "✅" if total >= 0 else "❌"
