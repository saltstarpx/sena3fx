...
from yagami_mtf_v67 import generate_signals, calculate_atr
...
def run_backtest(symbol, start_date, end_date, spread_pips):
    ...
    for i in range(len(data_1m)):
        ...
        if current_position != 0:
            # 半利確ロジック (リスク幅の1.5倍で実行 - RR5.0に合わせて少し伸ばす)
        ...
        if current_position == 0:
            ...
            if signal != 0:
                ...
                if current_position == 1:
                    half_tp = entry_price + (risk_amount * 1.5)
                else:
                    half_tp = entry_price - (risk_amount * 1.5)
...
if __name__ == "__main__":
    ...
    trades_v67 = run_backtest(SYMBOL, START_DATE, END_DATE, SPREAD_PIPS)

    output_dir = "/home/ubuntu/sena3fx/results"
    os.makedirs(output_dir, exist_ok=True)
    trades_v67.to_csv(os.path.join(output_dir, "trades_v67.csv"), index=False)

    print(f"Backtest completed. Results saved to {os.path.join(output_dir, 'trades_v67.csv')}")

    if not trades_v67.empty:
        total_pnl = trades_v67['PnL_pips'].sum()
        num_trades = len(trades_v67[~trades_v67['Position'].str.contains('HALF')]) 
        winning_trades = trades_v67[trades_v67['PnL_pips'] > 0]
        losing_trades = trades_v67[trades_v67['PnL_pips'] < 0]
