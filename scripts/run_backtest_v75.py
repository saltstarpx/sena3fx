...
run_backtest_v75.py
===================
v75戦略バックテスト: 4h+1h 二番底・二番天井 + 1分足始値で成行エントリー
"""
import pandas as pd
import numpy as np
import sys, os
...
if __name__ == "__main__":
    ...
    print("\nバックテスト実行中（1分足でSL/TP判定）...")
    df_trades = run_backtest(data_1m, signals)
    df_trades.to_csv(f"{OUT}/trades_v75.csv", index=False)

    stats = print_stats(df_trades, f"v75 | {SYMBOL} {START}〜{END} spread={SPREAD}pips RR={RR_RATIO} [1分足エントリー]")
    plot_equity(df_trades, f"v75 (4h+1h 二番底・二番天井 / 1分足エントリー) RR{RR_RATIO}", f"{OUT}/v75_equity_curve.png")

    # v74との比較
    print("\n\n" + "="*60)
    print("  v74 vs v75 比較（15分足エントリー vs 1分足エントリー）")
    print("="*60)
    try:
        df_v74 = pd.read_csv(f"{OUT}/trades_v74.csv")
        v74_total = df_v74["pnl"].sum()
        v74_closed = df_v74[df_v74["type"].isin(["SL", "TP"])]
        v74_wins = df_v74[df_v74["pnl"] > 0]
        v74_losses = df_v74[df_v74["pnl"] < 0]
        v74_wr = len(v74_wins) / (len(v74_wins) + len(v74_losses)) * 100 if (len(v74_wins) + len(v74_losses)) > 0 else 0
        v74_pf = v74_wins["pnl"].sum() / abs(v74_losses["pnl"].sum()) if len(v74_losses) > 0 else float("inf")
        print(f"  {'指標':<15} {'v74 (15分足)':<20} {'v75 (1分足)':<20}")
        print(f"  {'-'*55}")
        print(f"  {'総損益':<15} {v74_total:+.1f} pips{'':<8} {stats.get('total', 0):+.1f} pips")
        print(f"  {'エントリー数':<15} {len(v74_closed)}回{'':<14} {stats.get('entries', 0)}回")
        print(f"  {'勝率':<15} {v74_wr:.1f}%{'':<14} {stats.get('wr', 0):.1f}%")
        print(f"  {'PF':<15} {v74_pf:.2f}{'':<15} {stats.get('pf', 0):.2f}")
        print(f"  {'ケリー基準':<15} 0.3253{'':<14} {stats.get('kelly', 0):.4f}")
    except Exception as e:
        print(f"  v74データ読み込みエラー: {e}")
