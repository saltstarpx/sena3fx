"""
backtest_is_period.py
=====================
IS期間（2024-12-01 〜 2025-02-28）バックテスト
OOS期間（2025-03-03 〜 2025-06-30）との比較用

backtest_multi_fast.py と同一ロジック・手数料処理を使用。
"""
...
# ── メイン ────────────────────────────────────────────────
print("=" * 70)
print(f"複数銘柄同時バックテスト【IS期間】[AdaptiveRiskManager + PositionManager]")
print(f"期間: {IS_START} 〜 {IS_END}  初期資金: {INIT_CASH:,}円  基本リスク: {BASE_RISK*100:.0f}%")
...
for sym in SYMBOLS:
    sym_lower = sym.lower()
    # IS期間データを優先、なければ通常ファイルを使用
    d1m  = load_csv(os.path.join(DATA_DIR, f"{sym_lower}_is_1m.csv"))
    d15m = load_csv(os.path.join(DATA_DIR, f"{sym_lower}_is_15m.csv"))
    d4h  = load_csv(os.path.join(DATA_DIR, f"{sym_lower}_is_4h.csv"))

    if d1m is None:
        d1m  = load_csv(os.path.join(DATA_DIR, f"{sym_lower}_1m.csv"))
    if d15m is None:
        d15m = load_csv(os.path.join(DATA_DIR, f"{sym_lower}_15m.csv"))
    if d4h is None:
        d4h  = load_csv(os.path.join(DATA_DIR, f"{sym_lower}_4h.csv"))

    if d1m is None or d15m is None or d4h is None:
        print(f"  {sym}: データなし → スキップ")
        continue

    d1m  = slice_period(d1m,  IS_START, IS_END)
    d15m = slice_period(d15m, IS_START, IS_END)
    d4h  = slice_period(d4h,  IS_START, IS_END)

    if d1m is None or len(d1m) == 0:
        print(f"  {sym}: IS期間のデータなし → スキップ")
        continue

    arm_ref = AdaptiveRiskManager(sym, base_risk_pct=BASE_RISK)
    print(f"  {sym}: シグナル生成中... (spread={arm_ref.spread_pips}pips, データ行数={len(d1m)})")
...
print(f"\n{'='*70}")
print(f"【IS期間バックテスト 結果サマリー（{IS_START} 〜 {IS_END}）】")
print(f"{'='*70}")
...
fig.suptitle(
    f"複数銘柄同時バックテスト【IS期間】\n"
    f"銘柄: {', '.join(SYMBOLS)}  期間: {IS_START}〜{IS_END}  初期資金: {INIT_CASH:,}円",
...
if len(accepted) > 0:
    ...
    x = range(len(SYMBOLS))
    ax.bar(x, sym_counts, color="#6366f1", alpha=0.8)
    ax.set_xticks(x)
...
plt.tight_layout()
out_path = os.path.join(OUT_DIR, "is_period_result.png")
...
# CSV保存
if len(accepted) > 0:
    csv_path = os.path.join(OUT_DIR, "is_period_trades.csv")
