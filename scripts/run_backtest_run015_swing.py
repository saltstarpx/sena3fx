...
from strategies.yagami_mtf_v60_swing import generate_signals

def run_backtest(csv_path, spread_pips=0.2):
    ...
    print("Generating signals (Swing Edition)...")
    signal_series, tp_series, sl_series, signals_list = generate_signals(data_15m, data_1h, data_4h, spread)
    ...
    for signal in signals_list:
        ...
        entry_price = data_15m.loc[entry_time, 'close']
        tp = tp_series.loc[entry_time]
        sl = sl_series.loc[entry_time]
        
        post_entry_data = data_15m.loc[entry_time:]
        pnl = 0
    ...
    print("="*80)
    print(f"【RUN-015 Swing】結果 (Spread: {spread_pips} pips)")
    ...
    pd.DataFrame(trades).to_csv(f"/home/ubuntu/sena3fx/results/run015_swing_{os.path.basename(csv_path)}_trades.csv", index=False)
