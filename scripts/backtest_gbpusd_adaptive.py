"""
backtest_gbpusd_adaptive.py
============================
GBPUSD 1銘柄集中バックテスト（アダプティブリスク 2〜3%）
FX銘柄 Goldロジック順位 第2位（OOS PF=1.86）の検証

【設定】
  - 銘柄: GBPUSD のみ
  - スプレッド: 0.1pips（OANDAプロコース生スプレッド）
...
# ── コスト設定（OANDAプロコース） ────────────────────────────────
SPREAD_PIPS          = 0.1    # 生スプレッド 0.1pips（GBPUSD）
...
def main():
    print("\n" + "="*80)
    print("  GBPUSD 集中戦略バックテスト（アダプティブリスク 2〜3%, 0.5%刻み）")
    print(f"  FX Goldロジック 第2位（OOS PF=1.86）")
    ...
    d15m_full, d4h_full = load_gbpusd()
    ...
    # ── NZDUSD との比較 ──────────────────────────────────────────
    print("\n" + "-"*80)
    print("  ■ NZDUSD比較（OOS アダプティブ）")
    print(f"  {'銘柄':>8} {'PF':>8} {'MDD':>8} {'倍率':>8} {'最終資産':>12}")
    print(f"  {'NZDUSD':>8} {'2.74':>8} {'24.2%':>8} {'68.0x':>8} {'6,801万':>12}  (参考)")
    if oos_ada:
        pf_s = f"{oos_ada['pf']:.2f}" if oos_ada['pf'] < 99 else "∞"
        print(f"  {'GBPUSD':>8} {pf_s:>8} {oos_ada['mdd']:.1f}%{'':<3} "
              f"{oos_ada['multiplier']:.1f}x{'':<3} {oos_ada['final_eq']/10000:.0f}万{'':>4}")
