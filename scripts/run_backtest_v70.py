...
from yagami_mtf_v70 import generate_signals
...
def run_backtest(symbol, start_date, end_date, spread_pips):
    ...
    for i in range(len(data_1m)):
        ...
        if pos == 0:
            ...
            if sig != 0:
                entry_price = bar["open"]  # 成行 = 始値
                entry_time = t; pos = sig
...
if __name__ == "__main__":
    ...
    trades.to_csv(f"{OUT}/trades_v70.csv", index=False)
    print_stats(trades, f"v70 (成行) | {SYMBOL} {START}〜{END} spread={SPREAD}pips")
    plot_equity(trades, "Yagami MTF v70 (成行エントリー)", f"{OUT}/v70_equity_curve.png")

    # 個別トレード表示
    if not trades.empty:
        entries_only = trades[~trades["Position"].str.contains("HALF")]
        print(f"\n  個別トレード一覧:")
        print(entries_only[["EntryTime","Position","EntryPrice","ExitPrice","PnL_pips","Type"]].to_string(index=False))
