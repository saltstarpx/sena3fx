...
- **現在の戦略バージョン**: **v79**（2026/3/9 正式採用、定量・計量分析に基づくMDD対策+トレンドフォロー強化）
...
## 現在の戦略: v79（FX + METALS カテゴリ別フィルター）/ v77（USDJPY）
...
│   │   ├── yagami_mtf_v77.py        # v77（USDJPY用、1m足なしのためv79対象外）
...
│   └── strategies/
│       ├── yagami_mtf_v79.py        # 本番用v79（current/と同一、FX/METALS用）
│       ├── yagami_mtf_v78.py        # 参照用v78（current/と同一）
│       ├── yagami_mtf_v77.py        # 本番用v77（current/と同一、USDJPY用）
│       └── archive/
│
...
│   ├── backtest_v79_mdd_trend.py        # ★ v79定量・計量分析+改善バックテスト（最新）
...
```bash
# v79 定量・計量分析 + 改善バックテスト（最新）
python3.11 scripts/backtest_v79_mdd_trend.py
