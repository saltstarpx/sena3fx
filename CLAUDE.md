...
| **GOLDYAGAMI** | v79 | Goldロジック / Logic-A | オーパーツYAGAMIに「引き算の美学（ADX/セッション除去）+ 足し算の効果（日足EMA20+EMA距離）」を施したロジック。NZDUSD/XAUUSD/AUDUSDで現行 |
| **v80 Purified** | v80 | KMID+KLOW+Body | 指標予測力分析により不安定フィルター（日足EMA/ADX/Streak/EMA距離）を除去、安定2指標（KMID+4Hボディ比率）のみ残した統一ロジック。3.0R。GBPUSD/EURUSD/USDCADで採用 |
...
### v80 Purified の確定フィルター（GBPUSD/EURUSD/USDCAD）

```
フィルター（3つのみ）:
  ✅ KMID         — 4H文脈足 実体方向一致（r=+0.122, 77%期間一貫）
...
除去確定（指標予測力分析で不安定と判明）:
  ...
  ❌ Streak（予測力ほぼゼロ）
  ❌ EMA距離（弱すぎて有意でない）
  ❌ セッション時間 / ATR拡張 / 確認足方向チェック
```

### GOLDYAGAMI の確定フィルター（NZDUSD/XAUUSD）

```
  ✅ KMID + KLOW + EMA距離 + 日足EMA20方向一致
  RR = 2.5R（半利確あり）
```
...
|---|---|---|:---:|
| **v80** | 2026/3/15 | 指標予測力分析→不安定フィルター除去（KMID+KLOW+Body統一）、3.0R | GBPUSD **2.21** / EURUSD **2.61** / USDCAD **3.31** |
...
## 現在の採用銘柄・戦略（2026/3/15更新、同一エンジン検証済み）

| 銘柄 | 戦略 | PF | Sharpe | MDD | Kelly | RR | tol | OOS/IS | 月+ | 状態 |
|---|---|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| **USDCAD** | **v80 KMID+KLOW+Body** | **3.32** | **5.70** | **12.5%** | **0.504** | **3.0** | 0.30 | **1.60** | **10/10** | ✅本番 |
| **XAUUSD** | Logic-A GOLD tol=0.20 | **3.13** | 3.85 | **8.7%** | 0.511 | 2.5 | 0.20 | 1.20 | 9/10 | ✅本番 |
| **EURUSD** | **v80 KMID+KLOW+Body** | **2.61** | **5.70** | **23.5%** | **0.434** | **3.0** | 0.30 | **1.17** | **10/10** | ✅本番 |
| **AUDUSD** | **v80 KMID+KLOW+Body** | **2.52** | **5.90** | **16.7%** | **0.408** | **3.0** | 0.30 | **1.27** | **10/10** | ✅本番 |
| **GBPUSD** | **v80 KMID+KLOW+Body** | **2.24** | **6.39** | **20.0%** | **0.394** | **3.0** | 0.30 | **0.85** | **10/10** | ✅本番 |
| **USDCHF** | **v80 KMID+KLOW+Body** | **2.23** | **8.75** | **11.9%** | **0.378** | **3.0** | 0.30 | **1.04** | **9/9** | ✅本番 |
| **NZDUSD** | Logic-A GOLD tol=0.20 | **2.11** | 3.35 | **14.6%** | 0.337 | **3.0** | 0.20 | 0.73 | 9/10 | ✅本番 |
| **USDJPY** | v77 Logic-C（オーパーツ） | **1.83** | 5.70 | 20.0% | 0.300 | **3.0** | 0.30 | 0.79 | 10/10 | ✅本番 |

> **同一エンジン検証（2026/3/15）**: `backtest_final_optimized.py` で全8銘柄 × 3バリアント（現行2.5R / 現行3.0R / v80 3.0R）を比較。OOS PF最高かつOOS/IS≥0.70のロジックを採用。
> **v80採用拡大**: AUDUSD（旧Logic-B）もv80が最強と判明。5銘柄がv80統一ロジック。
> **3.0R統一**: 7/8銘柄がRR=3.0R。XAUUSDのみ2.5R維持（3.0Rで PF低下）。
> **v80不採用**: XAUUSD（PF低下）、NZDUSD（現行3.0Rの方がPF高い）、USDJPY（OOS/IS=0.41 過学習）。
...
### 銘柄別最強ロジック（2026/3/15 同一エンジン検証確定）

| カテゴリ | 銘柄 | ロジック | PF | RR | tol | OOS/IS | 備考 |
|---|---|:---:|:---:|:---:|:---:|:---:|---|
| FX | **USDCAD** | **v80 KMID+KLOW+Body** | **3.32** | **3.0** | 0.30 | 1.60 | PF +70%, MDD 22.0→12.5% |
| 貴金属 | **XAUUSD** | Logic-A GOLD | **3.13** | 2.5 | 0.20 | 1.20 | v80/3.0RともにPF低下 |
| FX | **EURUSD** | **v80 KMID+KLOW+Body** | **2.61** | **3.0** | 0.30 | 1.17 | PF +29% |
| FX | **AUDUSD** | **v80 KMID+KLOW+Body** | **2.52** | **3.0** | 0.30 | 1.27 | 旧Logic-B→v80に変更、Sharpe 3.83→5.90 |
| FX | **GBPUSD** | **v80 KMID+KLOW+Body** | **2.24** | **3.0** | 0.30 | 0.85 | PF +22% |
| FX | **USDCHF** | **v80 KMID+KLOW+Body** | **2.23** | **3.0** | 0.30 | 1.04 | Sharpe=8.75（最高） |
| FX | **NZDUSD** | Logic-A GOLD | **2.11** | **3.0** | 0.20 | 0.73 | v80よりPF高い（2.11>2.08） |
| FX | **USDJPY** | Logic-C オーパーツ(v77) | **1.83** | **3.0** | 0.30 | 0.79 | v80はOOS/IS=0.41で過学習 |
...
### 2026-03-15 セッション完了内容（第2回: 同一エンジン検証）

- **同一バックテストエンジンでの公平比較**: `backtest_final_optimized.py` にv80ロジック追加
  - 全8銘柄 × 3バリアント（現行2.5R / 現行3.0R / v80 3.0R）を同一条件比較
  - 前回セッションでは `backtest_v80_purified.py`（別シグナル生成関数・E0統一）の結果を使っていたが、
    本番エンジンと一致する `backtest_final_optimized.py` で再検証
- **v80採用拡大**: AUDUSD を旧Logic-B → v80 3.0Rに変更（PF 2.47→2.52, Sharpe 3.83→5.90）
- **NZDUSD修正**: v80（PF 2.08）より現行Logic-A 3.0R（PF 2.11）の方が強いことが判明。v80→Logic-A 3.0Rに戻す
- **3.0R統一**: 7/8銘柄がRR=3.0R。XAUUSDのみ2.5R維持（3.0RでPF 3.13→3.05に低下）
- **v80不採用確認**: XAUUSD（PF低下）、USDJPY（OOS/IS=0.41 過学習）
- APPROVED_UNIVERSE全銘柄のPF/Kelly/noteを同一エンジンの値に統一
- README.md全面更新（8銘柄体制・ロジック解説・デプロイ手順）

### 2026-03-15 セッション完了内容（第1回）

- **v80 Purified 戦略採用**: 指標予測力分析→安定フィルターのみ残す「引き算の第二弾」
  - 全7銘柄×5バリアント×2 RR のバックテスト実施（`scripts/backtest_v80_purified.py`）
  - KMID（r=+0.122, 77%期間一貫）+ 4Hボディ比率（r=+0.052, 60%一貫）のみ安定
  - 日足EMA方向一致は**逆効果**（match WR 26.9% < no-match 30.1%）と判明
  - ADX/Streak/EMA距離は不安定（期間で効果反転）
- **4銘柄をv80に切替**:
  - USDCAD: PF 1.93→**3.31**（+71%）、MDD 25.5→12.8%
  - EURUSD: PF 2.04→**2.61**（+28%）、OOS/IS=1.17
  - GBPUSD: PF 1.79→**2.21**（+23%）、OOS/IS=0.86
  - USDCHF: PF **1.95**（新規追加、月次安定型・直近3M占有25.7%）
- **4銘柄は現行維持**:
  - NZDUSD: v80でMDD悪化(+5pp)、改善幅小さい
  - XAUUSD: v80でPF微減、現行が既に優秀(3.04)
  - AUDUSD: v80でMDD大幅悪化(+7.7pp)
  - USDJPY: v80でOOS/IS=0.41（過学習）
- **RR比率の銘柄別化**: v80銘柄は3.0R、他は2.5R維持（strategy_params経由）
- **USDCHF新規追加**: 非本番10銘柄v80バックテストでXAGUSD(PF2.76)とUSDCHF(PF1.95)が候補
  - XAGUSD: 直近3M利益占有率40.7%（金銀上昇トレンド依存）→ 不採用
  - USDCHF: 直近3M占有率25.7%、14ヶ月中12ヶ月プラス → 月次安定型で採用
- **ポートフォリオ**: 7→8銘柄体制、MAX_OPEN_POSITIONS=8
- 新規スクリプト: `scripts/backtest_v80_purified.py`, `scripts/analyze_indicator_predictive_power.py`
- 結果CSV: `results/backtest_v80_purified.csv`
...
- **USDJPY本番復帰**: 1m足から4hデータ再生成→PF 2.02 / Sharpe 7.83で採用基準クリア。APPROVED_UNIVERSEに追加
