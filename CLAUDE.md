...
## 現在の採用銘柄・戦略（2026/3/15更新）

| 銘柄 | 戦略 | PF | Sharpe | MDD | Kelly | RR | tol | 月+ | 状態 |
|---|---|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| **USDCAD** | **v80 KMID+KLOW+Body** | **3.31** | **5.52** | **12.8%** | **0.504** | **3.0** | 0.30 | **10/10** | ✅本番 |
| **EURUSD** | **v80 KMID+KLOW+Body** | **2.61** | **5.70** | **23.5%** | **0.434** | **3.0** | 0.30 | **10/10** | ✅本番 |
| **AUDUSD** | v79BC Logic-B（ADX+Streak+4Hボディ≥0.3） | 2.49 | 4.85 | 18.9% | 0.353 | 2.5 | 0.30 | 14/14 | ✅本番 |
| **XAUUSD** | v79A Logic-A（GOLD）tol=0.20 | 2.46 | 3.87 | 12.6% | 0.409 | 2.5 | 0.20 | 12/14 | ✅本番 |
| **GBPUSD** | **v80 KMID+KLOW+Body** | **2.21** | **6.24** | **20.2%** | **0.388** | **3.0** | 0.30 | **10/10** | ✅本番 |
| **NZDUSD** | v79A Logic-A（GOLD）tol=0.20 | 2.14 | 5.88 | 12.8% | 0.368 | 2.5 | 0.20 | 14/14 | ✅本番 |
| **USDJPY** | v77 Logic-C（オーパーツ） | 2.02 | 7.83 | 20.4% | 0.337 | 2.5 | 0.30 | 9/9 | ✅本番 |
| **USDCHF** | **v80 KMID+KLOW+Body** | **1.95** | **4.32** | **21.8%** | **0.281** | **3.0** | 0.30 | **12/14** | ✅本番 |

> **v80採用（2026/3/15）**: 指標予測力分析により、安定した予測力を持つフィルター（KMID r=+0.122 / 4Hボディ比率 r=+0.052）のみ残し、不安定フィルター（日足EMA/ADX/Streak/EMA距離）を除去。GBPUSD/EURUSD/USDCAD/USDCHFの4銘柄で採用。NZDUSD/XAUUSD/AUDUSD/USDJPYは現行維持（v80でMDD悪化or OOS/IS過学習）。
> **USDCHF新規追加（2026/3/15）**: 月次PnL分布が安定（直近3M占有率25.7%）。XAGUSDはPF高いが直近3M占有40.7%でトレンド依存リスクあり不採用。
...
### カテゴリ別推奨バリアント（2026/3/15 更新）

| カテゴリ | 銘柄 | ロジック | PF | RR | tol | 備考 |
|---|---|:---:|:---:|:---:|:---:|---|
| FX | **USDCAD** | **v80 KMID+KLOW+Body** | **3.31** | **3.0** | 0.30 | PF +71%, MDD 25.5→12.8% |
| FX | **EURUSD** | **v80 KMID+KLOW+Body** | **2.61** | **3.0** | 0.30 | PF +28%, OOS/IS=1.17 |
| FX | **GBPUSD** | **v80 KMID+KLOW+Body** | **2.21** | **3.0** | 0.30 | PF +23%, OOS/IS=0.86 |
| FX | **USDCHF** | **v80 KMID+KLOW+Body** | **1.95** | **3.0** | 0.30 | 月次安定型（直近3M占有25.7%）、新規追加 |
| 貴金属 | XAUUSD | Goldロジック | 2.46 | 2.5 | 0.20 | MDD 12.6%, 現行維持 |
| FX | NZDUSD | Goldロジック | 2.14 | 2.5 | 0.20 | MDD 12.8%, 現行維持 |
| FX | AUDUSD | ADX+Streak+4Hボディ≥0.3 | 2.49 | 2.5 | 0.30 | Logic-B, 現行維持 |
| FX | USDJPY | オーパーツ(v77) | 2.02 | 2.5 | 0.30 | Logic-C, 現行維持（v80はOOS/IS=0.41で不採用） |
