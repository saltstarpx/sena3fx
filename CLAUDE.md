...
│   ├── backtest_category_improvements.py # ★ カテゴリ別過学習チェック（FX/METALS/INDICES）
│   ├── backtest_v78_improvements.py     # v78改善段階検証（v77→v78A/B/C/D、XAUUSD専用）
...
```bash
# カテゴリ別過学習チェック（FX/METALS/INDICES × 全改善バリアント）
python3.11 scripts/backtest_category_improvements.py
...
| **XAUUSD** | v77 | **2.03** | 15.6% | -0.012 | 12/12 |

次の候補: なし（カテゴリ別検証でFX/指数は採用基準未達を確認）

### カテゴリ別推奨バリアント（2026/3/9 カテゴリ検証結果）

| カテゴリ | 銘柄 | 推奨バリアント | セッション | avg OOS PF | 備考 |
|---|---|:---:|:---:|:---:|---|
| FX | EURUSD/GBPUSD/AUDUSD | **v77+改善B** | London+NY (UTC7-22) | 1.92 | PF<2.0のため採用基準未達 |
| 貴金属 | XAUUSD | **v77** | — | 2.03 | セッションフィルター逆効果 |
| 指数 | US30/SPX500/NAS100 | なし | — | <1.2 | 全バリアントで採用不可 |
...
UTC5-21というパラメータはXAUUSD OOS期間のデータから逆算された可能性が高く、
汎用性の観点では**v77が貴金属の推奨戦略**となる。

`strategies/current/yagami_mtf_v78.py` および `cloud_run/strategies/yagami_mtf_v78.py` は
参照用に保持するが、**現行本番運用はv77を使用する**。
