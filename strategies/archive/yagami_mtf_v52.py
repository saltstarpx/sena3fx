...
def generate_signals(data_1m, data_15m, data_4h, spread=0.01):
    ...
    for i in range(len(data_15m)):
        ...
        # ロング：4H上昇トレンド中、15分足で下髭（ATRの0.3倍以上）を確認
        long_condition = (current_4h_trend == 1) and ((current_m15_bar["open"] - current_m15_bar["low"]) > (atr_val * 0.3))
        
        # ショート：4H下落トレンド中、15分足で上髭（ATR_0.3倍以上）を確認
        short_condition = (current_4h_trend == -1) and ((current_m15_bar["high"] - current_m15_bar["open"]) > (atr_val * 0.3))
        ...
        if long_condition:
            ...
            for _, bar in entry_1m_bars.iterrows():
                # 髭の50%戻し付近でエントリー
                if bar['low'] <= (current_m15_bar['low'] + (current_m15_bar['open'] - current_m15_bar['low']) * 0.5):
                    sl_price = current_m15_bar['low'] - (atr_val * 0.5) # 損切りを離す
            ...
            for _, bar in entry_1m_bars.iterrows():
                # 髭の50%戻し付近でエントリー
                if bar['high'] >= (current_m15_bar['high'] - (current_m15_bar['high'] - current_m15_bar['open']) * 0.5):
                    sl_price = current_m15_bar['high'] + (atr_val * 0.5) # 損切りを離す
