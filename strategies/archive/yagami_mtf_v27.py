#!/usr/bin/env python3
"""
やがみ式3層MTF戦略 v27 - 損切り緩和・根拠重視型

改善点：
1. 損切り設定: 15分足の髭先から「ATR(15m)の0.2倍」程度離した場所に置く。
2. エントリー条件: 15分足の髭を確認後、1分足で反転（陽線/陰線）を確認してから。
3. 決済ロジック: 
   - TP: エントリー価格からリスクと同じ値幅（RR=1.0）
   - これにより、低ボラティリティ環境でも現実的な利確を目指す。
4. 4Hトレンドフィルタ: 緩やかなトレンドでもエントリーを許可。
"""
...
def generate_signals(data_1m, data_15m, data_4h, spread=0.01):
    ...
    while i < len(data_1m):
        ...
        # ロングシグナル判定
        if h4_trend == 1 and lower_wick > body_size * 0.5 and lower_wick > atr_val * 0.05:
            # 1分足で反転（陽線）を確認
            if curr_bar["close"] > curr_bar["open"] and curr_bar["close"] > m15_low:
                sl_price = m15_low - (atr_val * 0.2) # 髭先からATRの0.2倍離す
                    ...
                    signals_list.append(
                        {
                            "time": curr_time,
                            "direction": "LONG",
                            "tp": curr_bar["close"] + risk * 1.0, # RR=1.0
                            "sl": sl_price,
                        }
                    )
                    i += 15 # 同じ15分足内での重複エントリー防止
                    continue
        # ショートシグナル判定
        elif h4_trend == -1 and upper_wick > body_size * 0.5 and upper_wick > atr_val * 0.05:
            # 1分足で反転（陰線）を確認
            if curr_bar["close"] < curr_bar["open"] and curr_bar["close"] < m15_high:
                sl_price = m15_high + (atr_val * 0.2) # 髭先からATRの0.2倍離す
