#!/usr/bin/env python3
"""
やがみ式3層MTF戦略 v16 - トレンドフィルタ適正化・15分足形成中監視版

改善点：
1. 4Hトレンド判定の適正化: EMA20の傾き（Slope）が0.005以上の場合のみに限定。
2. 15分足形成中のリアルタイム監視: 15分足が確定するのを待たず、15分足の枠内で「ATR 0.2倍以上の髭」が発生し、
...
def analyze_h4_environment(h4_data):
    ...
    h4_data['trend'] = 0
    h4_data.loc[(h4_data['close'] > h4_data['ema20']) & (h4_data['ema_slope'] > 0.005), 'trend'] = 1
    h4_data.loc[(h4_data['close'] < h4_data['ema20']) & (h4_data['ema_slope'] < -0.005), 'trend'] = -1
    return h4_data
...
def generate_signals(data_1m, data_15m, data_4h, spread=0.01):
    ...
    while i < len(data_1m):
        ...
        if h4_trend == 1 and lower_wick > atr_val * 0.2:
            if curr_bar['close'] > curr_bar['open']:
                signals_list.append({'time': curr_time, 'direction': 'LONG'})
                i += 15
                continue
        elif h4_trend == -1 and upper_wick > atr_val * 0.2:
            if curr_bar['close'] < curr_bar['open']:
