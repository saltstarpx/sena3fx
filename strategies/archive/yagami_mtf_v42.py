#!/usr/bin/env python3
"""
やがみ式3層MTF戦略 v42 - 1分足マシンガン・RR 5.0・全バーエントリー型

改善点：
1. RR 5.0 固定: 1回の利益で5回分の負けをカバー。
2. 取引回数の絶対確保: 
   - 1分足の各バーにおいて、4Hトレンド方向であれば「5分おき」に強制エントリー。
   - 15分足の髭などのフィルタを一切排除し、1月データでも数百回の取引を確実に行う。
3. 損切り設定: 前15分足の安値/高値から「ATR(15m)の0.2倍」離す（タイトめ）。
"""
...
def generate_signals(data_1m, data_15m, data_4h, spread=0.01):
    ...
    while i < len(data_1m):
        ...
        # ロングシグナル (トレンド方向へ5分おきにエントリー)
        if h4_trend == 1:
            sl_price = prev_m15["low"] - (atr_val * 0.2)
        ...
        # ショートシグナル
            ...
            sl_price = prev_m15["high"] + (atr_val * 0.2)
