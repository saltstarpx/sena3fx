#!/usr/bin/env python3
"""
やがみ式3層MTF戦略 v34 - リスク幅拡大・RR 5.0・高頻度型

改善点：
1. RR 5.0 固定: 1回の利益で5回分の負けをカバー。
2. リスク幅（損切り幅）の拡大: 
   - スプレッド1.0pipsの即狩りを防ぐため、損切りを髭先から「ATR(15m)の0.5倍」離す。
   - これにより、ノイズで切られにくくし、本物のトレンド転換まで耐える。
3. 髭判定の極限緩和: 取引回数100回以上を確保。
4. 4Hトレンド追随: EMA20との位置関係のみで判定し、エントリーチャンスを増やす。
"""
...
def generate_signals(data_1m, data_15m, data_4h, spread=0.01):
    ...
    while i < len(data_1m):
        ...
        if h4_trend == 1:
            # 1分足が15分足の安値付近にある場合
            if curr_bar["close"] <= m15_low + (atr_val * 0.2):
                sl_price = m15_low - (atr_val * 0.5) # 髭先から十分に離す
                risk = curr_bar["close"] - sl_price
                if risk > 0.005: # スプレッドに対して十分なリスク幅
                    signals_list.append(
                        {
                            "time": curr_time,
                            "direction": "LONG",
        ...
        # ショートシグナル判定
            ...
            # 1分足が15分足の高値付近にある場合
            if curr_bar["close"] >= m15_high - (atr_val * 0.2):
                sl_price = m15_high + (atr_val * 0.5) # 髭先から十分に離す
