#!/usr/bin/env python3
"""
やがみ式3層MTF戦略 v36 - 極限高頻度・RR 5.0・損切り緩和型

改善点：
1. RR 5.0 固定: 1回の利益で5回分の負けをカバー。
2. 取引回数の絶対確保: 
   - 15分足の全バーにおいて、4Hトレンド方向であればエントリーを検討。
   - 髭の有無にかかわらず、15分足の安値/高値付近（ATRの0.5倍以内）にいれば即エントリー。
3. 損切り設定: 15分足の安値/高値から「ATR(15m)の0.5倍」離す。
"""
...
def generate_signals(data_1m, data_15m, data_4h, spread=0.01):
    ...
    while i < len(data_1m):
        ...
        # ロングシグナル判定 (安値付近でエントリー)
        if h4_trend == 1:
            if curr_bar["close"] <= m15_low + (atr_val * 0.5):
                ...
                if risk > 0.005:
                    ...
                    i += 15
                    continue
        # ショートシグナル判定 (高値付近でエントリー)
        elif h4_trend == -1:
            if curr_bar["close"] >= m15_high - (atr_val * 0.5):
