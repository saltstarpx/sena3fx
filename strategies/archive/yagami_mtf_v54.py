    ...
    for i in range(len(data_15m)):
        ...
        if long_condition:
            ...
            for _, bar in entry_1m_bars.iterrows():
                # 指値エントリー：髭の先端付近（90%戻し）まで引きつける
                limit_price = current_m15_bar['low'] + (current_m15_bar['open'] - current_m15_bar['low']) * 0.1
                if bar['low'] <= limit_price:
                    sl_price = current_m15_bar['low'] - (atr_val * 0.5) # 背を離す
            ...
            for _, bar in entry_1m_bars.iterrows():
                # 指値エントリー：髭の先端付近（90%戻し）まで引きつける
                limit_price = current_m15_bar['high'] - (current_m15_bar['high'] - current_m15_bar['open']) * 0.1
                if bar['high'] >= limit_price:
                    sl_price = current_m15_bar['high'] + (atr_val * 0.5) # 背を離す
