...
yagami_mtf_v74.py
================
やがみnote準拠: 4時間足 + 1時間足 の二番底・二番天井ロジック

【コンセプト】
- v73の4時間足限定から、1時間足も対象に追加してエントリー回数を増加
- 4時間足の二番底・二番天井: 更新直後の15分足2本以内でエントリー
- 1時間足の二番底・二番天井: 更新直後の15分足1本以内でエントリー
- 上位足（4時間足EMA20）のトレンド方向に沿ったシグナルのみ採用
- 損切りは二番底の安値 / 二番天井の高値の少し外側
- RR 2.5 固定

【優先順位】
1. 4時間足の二番底・二番天井（より強いシグナル）
2. 1時間足の二番底・二番天井（4時間足トレンドに沿ったもののみ）
"""
    ...
    for i in range(2, len(h1_times)):
        ...
        if trend == 1:
            ...
            if abs(low1 - low2) <= tolerance and h1_prev1["close"] > h1_prev1["open"]:
                sl = min(low1, low2) - atr_val * 0.15
                # 1時間足更新直後の15分足1本以内（15分以内）
                entry_window_end = h1_current_time + pd.Timedelta(minutes=15)
                m15_window = data_15m[
                    (data_15m.index >= h1_current_time) &
                    (data_15m.index < entry_window_end)
                ]
                if len(m15_window) > 0:
                    entry_bar = m15_window.iloc[0]
                    ...
                    if entry_time not in used_times:
                        ep = entry_bar["open"] + spread
                        risk = ep - sl
                        if 0 < risk <= h4_atr * 2:  # 1時間足はリスク上限を4時間足ATR×2に制限
                            tp = ep + risk * rr_ratio
                            signals.append({
                                "time": entry_time,
                                "dir": 1,
