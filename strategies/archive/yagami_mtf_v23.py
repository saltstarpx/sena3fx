#!/usr/bin/env python3
"""
やがみ式3層MTF戦略 v23 - 実体ベースの壁・ミスプライス埋めエントリー（厳密版）

改善点：
1. 4Hトレンド判定の緩和: EMA20の傾き（Slope）が0.0001以上の場合のみに限定。
2. 15分足の髭判定: 髭の長さが実体の1.0倍以上、かつATRの0.05倍以上の場合を「強い髭」と定義。
3. ミスプライス埋めエントリー（厳密化）: 15分足で強い髭が出現した後、その髭が1分足で埋められる動き（髭の安値/高値を実体で更新する）を確認してからエントリー。
   - ロングの場合: 15分足の安値が髭として形成された後、1分足が陽線で確定し、その終値が15分足の安値より上にあり、かつ15分足の始値より下にある（髭を埋め戻す動き）。
   - ショートの場合: 15分足の高値が髭として形成された後、1分足が陰線で確定し、その終値が15分足の高値より下にあり、かつ15分足の始値より上にある（髭を埋め戻す動き）。
...
def generate_signals(data_1m, data_15m, data_4h, spread=0.01):
    ...
    while i < len(data_1m):
        ...
        if h4_trend == 1 and lower_wick > body_size * 1.0 and lower_wick > atr_val * 0.05:
            # ミスプライス埋めエントリー: 1分足が陽線で、終値が15分足の安値より上にあり、かつ15分足の始値より下にある
            if curr_bar["close"] > curr_bar["open"] and curr_bar["close"] > m15_low and curr_bar["close"] < m15_open:
                signals_list.append(
                    {
                        "time": curr_time,
                        "direction": "LONG",
        ...
        # ショートシグナル判定
            ...
            # ミスプライス埋めエントリー: 1分足が陰線で、終値が15分足の高値より下にあり、かつ15分足の始値より上にある
            if curr_bar["close"] < curr_bar["open"] and curr_bar["close"] < m15_high and curr_bar["close"] > m15_open:
                signals_list.append(
                    {
                        "time": curr_time,
                        "direction": "SHORT",
