#!/usr/bin/env python3
"""
やがみ式3層MTF戦略 v29 - 深い押し目・高RR・トレンド追随型

改善点：
1. エントリータイミング: 15分足の髭を確認後、1分足で「髭の70%」程度まで深く戻してからエントリー。
   これにより、スプレッド1.0pipsを考慮しても、髭先（損切り）までの距離を極限まで短くする。
2. 損切り設定: 15分足の髭先から「ATR(15m)の0.2倍」離す。
3. 利益確定: リスクの3.0倍（RR=3.0）に設定。
   - 低ボラティリティ環境でも、1回の勝ちで3回分の負けをカバーできるようにする。
"""
...
def generate_signals(data_1m, data_15m, data_4h, spread=0.01):
    ...
    while i < len(data_1m):
        ...
        if h4_trend == 1 and lower_wick > body_size * 0.5 and lower_wick > atr_val * 0.1:
            # 髭の70%まで深く戻してからエントリー
            deep_pullback = m15_low + (lower_wick * 0.3)
            if curr_bar["close"] <= deep_pullback and curr_bar["close"] > m15_low:
                sl_price = m15_low - (atr_val * 0.2)
                risk = curr_bar["close"] - sl_price
                if risk > 0.005: # 最小リスク幅を確保（スプレッド負け防止）
                    signals_list.append(
                        {
                            "time": curr_time,
                            "direction": "LONG",
                            "tp": curr_bar["close"] + risk * 3.0, # RR=3.0
                            "sl": sl_price,
                        }
                    )
                    i += 15
        ...
        # ショートシグナル判定
            ...
            # 髭の70%まで深く戻してからエントリー
            deep_pullback = m15_high - (upper_wick * 0.3)
            if curr_bar["close"] >= deep_pullback and curr_bar["close"] < m15_high:
