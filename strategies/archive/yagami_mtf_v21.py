#!/usr/bin/env python3
"""
やがみ式3層MTF戦略 v21 - BB利益確定（オフセットあり）・タイト損切り版

改善点：
1. 4Hトレンド判定の緩和: EMA20の傾き（Slope）が0.0001以上の場合のみに限定。
2. 15分足形成中のリアルタイム監視: 15分足の枠内で「ATR 0.05倍以上の髭」が発生した瞬間にエントリー。
3. タイト損切り: エントリー時の15分足の安値（ロング時）または高値（ショート時）を損切りラインに設定。
4. BB利益確定（オフセットあり）: BB(20, 2)の順張り方向のバンドに +2pips（ロング）または -2pips（ショート）したラインにタッチした瞬間に利益確定。
   これにより、スプレッド1.0pipsを差し引いてもプラスになる可能性を高める。
"""
...
def calculate_bb(df, period=20, std_dev=2):
