#!/usr/bin/env python3
"""
やがみ式3層MTF戦略 v31 - 髭戻り待機・RR 5.0・高頻度型

改善点：
1. RR設定: 5.0固定。
2. エントリータイミング: 15分足の髭を確認後、1分足で「髭の50%」程度まで戻してからエントリー。
   v30の80%戻しは厳しすぎたため、50%に緩和して取引回数を確保。
3. 損切り設定: 15分足の髭先から「ATR(15m)の0.2倍」離す。
4. 髭判定緩和: 取引回数100回以上を死守するため、髭の長さをATRの0.05倍以上に設定。
"""
...
def generate_signals(data_1m, data_15m, data_4h, spread=0.01):
    ...
    while i < len(data_1m):
        ...
        if h4_trend == 1 and lower_wick > body_size * 0.3 and lower_wick > atr_val * 0.05:
            # 髭の50%まで戻してからエントリー
            limit_price = m15_low + (lower_wick * 0.5)
        ...
        # ショートシグナル判定
            ...
            # 髭の50%まで戻してからエントリー
            limit_price = m15_high - (upper_wick * 0.5)
