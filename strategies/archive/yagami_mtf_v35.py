#!/usr/bin/env python3
"""
やがみ式3層MTF戦略 v35 - 超高頻度・RR 5.0・損切り緩和型

改善点：
1. RR 5.0 固定: 1回の利益で5回分の負けをカバー。
2. エントリー条件の極限緩和: 
   - 15分足の髭が少しでもあれば、1分足がその髭の範囲内にいる限り即エントリー。
   - フィルタを最小限にし、取引回数を最大化する。
3. 損切り設定: 15分足の髭先から「ATR(15m)の0.5倍」離す。
4. 4Hトレンド追随: EMA20との位置関係のみで判定。
"""
...
def generate_signals(data_1m, data_15m, data_4h, spread=0.01):
    ...
    while i < len(data_1m):
        ...
        # ロングシグナル判定 (下髭)
        if h4_trend == 1:
            lower_wick_limit = min(m15_open, m15_close)
            if curr_bar["close"] <= lower_wick_limit and curr_bar["close"] > m15_low:
        ...
        # ショートシグナル判定 (上髭)
            ...
            upper_wick_limit = max(m15_open, m15_close)
            if curr_bar["close"] >= upper_wick_limit and curr_bar["close"] < m15_high:
