#!/usr/bin/env python3
"""
やがみ式3層MTF戦略 v24 - 高RR・時間帯限定・ATR適応型

改善点：
1. 時間帯限定: ロンドン・ニューヨーク時間（8-20 UTC）のみエントリー。
2. ATR適応型決済: 
   - TP: エントリー時のATR(15m)の2.0倍
   - SL: エントリー時のATR(15m)の0.5倍
   - これにより、低ボラティリティ環境でも適切な値幅を狙い、高RR(4.0)を実現。
3. ミスプライス埋めエントリー: v23の厳密版を継続。
"""
                ...
                signals_list.append(
                    {
                        "time": curr_time,
                        "direction": "LONG",
                        "tp": curr_bar["close"] + atr_val * 2.0,
                        "sl": curr_bar["close"] - atr_val * 0.5,
                    }
                )
                i += 15
                continue
