...
def generate_signals(data_1m, data_15m, data_4h, spread=0.01):
    ...
    for i in range(len(data_15m)):
        ...
        # 精度の追求：4Hトレンド方向への「大きな髭」（ATRの0.5倍以上）
        long_condition = (current_4h_trend == 1) and ((current_m15_bar["open"] - current_m15_bar["low"]) > (atr_val * 0.5))
        short_condition = (current_4h_trend == -1) and ((current_m15_bar["high"] - current_m15_bar["open"]) > (atr_val * 0.5))
        ...
        if long_condition:
            ...
            for _, bar in entry_1m_bars.iterrows():
                # 指値エントリー：髭の先端付近（95%戻し）まで極限まで引きつける
                limit_price = current_m15_bar['low'] + (current_m15_bar['open'] - current_m15_bar['low']) * 0.05
            ...
            for _, bar in entry_1m_bars.iterrows():
                # 指値エントリー：髭の先端付近（95%戻し）まで極限まで引きつける
                limit_price = current_m15_bar['high'] - (current_m15_bar['high'] - current_m15_bar['open']) * 0.05
