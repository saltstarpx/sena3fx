#!/usr/bin/env python3
"""
やがみ式3層MTF戦略 v17 - トレンドフィルタ超緩和・15分足形成中監視版

改善点：
1. 4Hトレンド判定の超緩和: EMA20の傾き（Slope）が0.001以上の場合のみに限定。
2. 15分足形成中のリアルタイム監視: 15分足の枠内で「ATR 0.1倍以上の髭」が発生した瞬間にエントリー。
...
def generate_signals(data_1m, data_15m, data_4h, spread=0.01):
    ...
    while i < len(data_1m):
        ...
        if h4_trend == 1 and lower_wick > atr_val * 0.1:
            signals_list.append({'time': curr_time, 'direction': 'LONG'})
            i += 15
            continue
        elif h4_trend == -1 and upper_wick > atr_val * 0.1:
