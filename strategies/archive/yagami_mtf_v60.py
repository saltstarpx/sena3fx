            ...
            for _, bar in entry_1m_bars.iterrows():
                ...
                if bar["low"] <= mid_point:
                    ...
                    if risk > 0:
                        ...
                        debug_signal_log.write(f"[{str(bar.name)}] LONG Signal generated! Entry: {bar["close"]:.5f}, TP: {tp_series.loc[bar.name]:.5f}, SL: {sl_series.loc[bar.name]:.5f}, Risk: {risk:.5f}, N_Value: {n_value:.5f}\n")
                        break
            ...
            for _, bar in entry_1m_bars.iterrows():
                ...
                if bar["high"] >= mid_point:
                    ...
                    if risk > 0:
                        ...
                        entry_time_series.loc[bar.name] = bar.name # エントリー時間を記録
                        atr_at_entry_series.loc[bar.name] = atr_val # エントリー時のATRを記録
                        debug_signal_log.write(f"[{str(bar.name)}] SHORT Signal generated! Entry: {bar["close"]:.5f}, TP: {tp_series.loc[bar.name]:.5f}, SL: {sl_series.loc[bar.name]:.5f}, Risk: {risk:.5f}, N_Value: {n_value:.5f}\n")
                        break
