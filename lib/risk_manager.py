...
LivermorePyramidingSizer: リバモア式価格ベースピラミッティング（v16新設）
                          → 実装は lib/pyramiding_logic.py に分離（後方互換 re-export）
...
# ── ピラミッティングロジック (後方互換 re-export) ──────────────────
# 実装は lib/pyramiding_logic.py に分離済み
from lib.pyramiding_logic import LivermorePyramidingSizer  # noqa: F401
