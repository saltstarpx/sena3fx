# Claude Code 指示書 v10: mainマージ、フォワードテスト準備、そして戦略ダッシュボード構築
...
これらの発見を基に、v10では「mainへの統合」「エース戦略の信頼性検証」「知見のドキュメント化」そして「戦略進化の可視化」を効率的に進めます。
...
### Task 3: [全員] 戦略パフォーマンス・ダッシュボードの構築

- **背景:** Dinii社のZenn記事に触発されました。「見えないものは改善できない」という思想は、我々のクオンツ開発にも完全に適用できます。バックテスト結果が散在している現状を改善し、戦略の進化を定量的に追跡できる仕組みを構築します。
- **目的:** 全戦略のパフォーマンス指標（PF, Sharpe, MDD, WR, トレード数）を一元管理し、時系列で可視化する。
- **アクション:**
    1.  **データ収集層の構築:**
        - `lib/backtest.py`（またはバックテスト実行の共通部分）を修正し、バックテスト完了時に、結果指標を`results/performance_log.csv`というファイルに**自動で追記（append）**する機能を追加してください。
        - CSVの各行には、`timestamp`, `strategy_name`, `parameters`, `timeframe`, `sharpe_ratio`, `profit_factor`, `max_drawdown`, `win_rate`, `trades`を含めてください。
    2.  **可視化層の構築:**
        - `results/performance_log.csv`を読み込み、戦略のパフォーマンスを可視化するシンプルなHTMLダッシュボード`dashboard.html`を作成してください。
        - ダッシュボードには、**戦略ごと**に**Sharpe Ratio**と**Max Drawdown**の**時系列推移**を表示する折れ線グラフを（Plotly.jsなどを利用して）描画してください。
        - これにより、「v8でUnion戦略を作った → v9でSharpeが1.859→2.817に改善された」という進化の軌跡が一目で分かるようになります。
...
- **変更内容:**
    ...
    - バックテスト結果を自動記録する`performance_log.csv`と、それを可視化する`dashboard.html`を構築。
