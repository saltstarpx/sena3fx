...
## [2026-03-02] Claude Code指示書v19追加：本番稼働準備 - OANDA Practice環境での実発注テスト
...
v18では、最終戦略 `XAUUSD+Kelly(f=0.25)+ADX(>25)` が `strategies/main_strategy.py` として確立され、フォワードテスト監視スクリプトもこれに対応しました。また、不要な実験ファイルが削除され、ダッシュボードも最新化されました。これにより、バックテストフェーズは完全に完了しました。

### v19の目的：本番への最終ブリッジ

v19では、いよいよ**OANDAのPractice（デモ）環境に接続し、実際の発注テスト**を行います。これが成功すれば、理論上はいつでも本番口座に切り替え可能な状態になります。

### 主なタスク指示

1.  **OANDA API発注クライアント実装:**
    - `lib/oanda_client.py` を新設し、`oandapyV20` を使って新規注文・決済・ポジション取得を行うクライアントを実装します。
2.  **監視スクリプトへの発注機能統合:**
    - `monitors/forward_main_strategy.py` に `--live` オプションを追加。このオプションが指定された場合、`OandaClient` を呼び出して実際に発注・決済を行うようにします。
3.  **systemdによるデーモン化:**
    - 完成した監視・発注スクリプトをサーバー上で永続的に実行させるため、`deploy/sena3fx.service` というsystemdのサービスファイルを作成します。

### Claude Codeへの指示

`prompt_for_claude_code_v19.md`を読み、記載されたタスクを順次実行すること。完了時には、**Practice環境で実際にポジションが建ち、決済されること**を確認して報告するよう指示しています。


**[2026-03-02] v19指示書更新：APIキープレースホルダー形式**

- OANDA本口座の審査完了を待つため、`prompt_for_claude_code_v19.md`内のAPIキーとアカウントIDを`"YOUR_OANDA_API_KEY"`のようなプレースホルダー形式に修正。ユーザーが後から簡単に入力できるように準備。


**[2026-03-02] v19指示書更新：APIキーを.envファイル管理に変更**

- セキュリティ向上のため、APIキーを指示書直書きから`.env`ファイル管理に変更。
- `.gitignore`に`.env`が記載済みであることを確認。
- ユーザーにOANDAサイトでのアカウントID確認方法を案内。
