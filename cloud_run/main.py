...
APPROVED_UNIVERSE = {
    ...
    "USDJPY": {
        ...
        "tier":          1,
        "base_risk_pct": 0.02,                    # 2%（3000万超で1%に切替）
        "oos_pf":        1.83,
        "kelly":         0.30,
...
# 同時ポジション上限: 0 = 無制限（銘柄ごとに1ポジション制限は維持）
...
# ── Equity Cap（絶対安全装置）────────────────────────────────────
# ロットサイズ計算に使うエクイティの上限。
# 複利膨張によるロット巨大化を防ぐ唯一の安全装置。
# いかなる状況でも変更・解除しないこと。
EQUITY_CAP_JPY = 100_000_000  # 1億円

# ── 資産規模ベース リスク切替 ──────────────────────────────────────
# 3000万未満: 2%、3000万以上: 1%
EQUITY_RISK_TABLE = [
    (30_000_000, 0.01),   # 3000万〜: 1%
    (         0, 0.02),   # 〜3000万: 2%
]

def _equity_base_risk(equity_jpy: float) -> float:
    """資産規模に応じたベースリスク%を返す（3000万で切替）"""
...
def calc_units_from_risk(ep: float, sl: float, pip_size: float,
    ...
    equity_jpy = min(_get_equity_jpy(), EQUITY_CAP_JPY)
...
def calc_dynamic_risk_pct(pair: str, trades_df: pd.DataFrame,
    ...
    """
    資産規模ベースのリスク切替のみ。
    3000万未満: 2%、3000万以上: 1%。WR乗数なし。
    """
    equity = _get_equity_jpy()
    return _equity_base_risk(equity)
...
def run_cycle():
    ...
    if MAX_OPEN_POSITIONS == 0 or len(open_positions) < MAX_OPEN_POSITIONS:
    ...
    logger.info(f"cycle end: open={len(open_positions)}/{'∞' if MAX_OPEN_POSITIONS == 0 else MAX_OPEN_POSITIONS}")
    return {
        "status": "ok",
...
async def notify_test_endpoint():
    ...
    send_discord("", embeds=[{
        ...
        "fields": [
            ...
            {"name": "同時上限",    "value": f"`{'無制限' if MAX_OPEN_POSITIONS == 0 else str(MAX_OPEN_POSITIONS) + '件'}`", "inline": True},
            {"name": "リスク設定",  "value": "2%（3000万超で1%）", "inline": True},
